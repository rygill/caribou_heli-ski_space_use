...
plot(mb, allTerms = TRUE, xlab = 'HORSE', ylab = 'MULE')

a = plot(mb, allTerms = FALSE, select= c(2))
b = plot(mb, allTerms = FALSE, select= c(3))
c = plot(mb, allTerms = FALSE, select= c(4))
d = plot(mb, allTerms = FALSE, select= c(5))
e = plot(mb, allTerms = FALSE, select= c(6))
...
agrob = a.plot +
  ...
  labs(x="elevation (m)", y="s(elevation)") +  
  labs(title = "a)") +
...
library(cowplot)
# cowplot solution
gridPlot = plot_grid(agrob, bgrob, cgrob, dgrob, egrob, align = "h", ncol = 3)

ggsave(gridPlot, file="./figures/HELI_plot_gam.png",
       width = 2000,
       height=2000,
       units = "px",
       dpi = 300)
